...
|---|---:|---:|
| Registers per thread | 39 | 56 |
| Theoretical occupancy | 100% | 66.67% |
| Achieved occupancy | 98.74% | 64.75% |
| 1024³ median performance | 1.615 TFLOPS | **2.807 TFLOPS** |

The Register64 kernel uses more registers per thread to increase output reuse. This reduces theoretical and achieved occupancy, but the additional register reuse improves arithmetic efficiency enough to produce substantially higher throughput on the tested workload. This demonstrates that occupancy alone is not a sufficient performance target for GEMM optimization.
